...
class SpellNumber:
    ...
    def __init__(self, number):
        if SpellNumber.is_valid_number(number):
            self.spelling = self.spell_number(SpellNumber.cast_to_number(number))
        else:
            self.spelling = None
    ...
    def is_valid_number(num):
        return True if re.search(r"\-?\d*\.?\d+", str(num)) else False
    ...
    def digit_length(num):
        return math.floor(math.log(num, 10) + 1) if num != 0 else 1
...
def main():
    ...
    spell_number_instance = SpellNumber(args.number)
    if spell_number_instance.spelling:
        print(spell_number_instance)
    else:
        print(f'"{args.number}" is not a valid number.')
